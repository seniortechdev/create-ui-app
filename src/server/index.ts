import { ApolloServerPluginCacheControl, ApolloServerPluginDrainHttpServer, gql } from 'apollo-server-core';
...
const resolvers = {
  State: {
    tradeSummary: async (parent: State) => {
      const { id } = parent;
      const response = await fetch(`https://datausa.io/api/data?Origin%20State=${id}&measure=Millions%20Of%20Dollars,Thousands%20Of%20Tons&drilldowns=Destination%20State&year=latest`);
      const data: any = await response.json();
      const results: InterstateTradeSummary[] = data.data;
      const totalTons = results.reduce((totalTons, result) => (totalTons += result['Thousands Of Tons']), 0);
      const totalDollarAmount = results.reduce((totalTons, result) => (totalTons += result['Millions Of Dollars']), 0);
    ...
    employmentSummary: async (parent: State) => {
      const { id } = parent;
      const response = await fetch(`https://datausa.io/api/data?Geography=${id}&measure=Total%20Population,Average%20Wage&drilldowns=Industry%20Group&Year=latest`);
      const data: any = await response.json();
    ...
    productionSummary: async (parent: State) => {
      const { id } = parent;
      const response = await fetch(`https://datausa.io/api/data?Origin%20State=${id}&measure=Millions%20Of%20Dollars,Thousands%20Of%20Tons&drilldowns=SCTG2&year=latest`);
      const data: any = await response.json();
  ...
  Query: {
    states: async (parent: unknown, args: { name?: string }) => {
      ...
      if (states) {
        if (name) {
          return states.filter((state: { name: string }) => state.name.toLowerCase().startsWith(name.toLowerCase()));
        }
        return states;
      } else {
        const response = await fetch('https://datausa.io/api/searchLegacy?dimension=Geography&hierarchy=State&limit=50000');
        const data: any = await response.json();
        if (name) {
          return data.results.filter((state: { name: string }) => state.name.toLowerCase().startsWith(name.toLowerCase()));
        }
        states = data.results;
        return data.results;
      }
...
passport.use(
  new LocalStrategy({ usernameField: 'username', passwordField: 'password' }, async (username, password, done) => {
    const userStmt = await User.query().where('username', '=', username).where('password', '=', password).execute();
